...
import { ArrowLeft, Wallet, RefreshCw, Calendar } from "lucide-react";
...
export default function HistorialEntradasPage({ user }) {
  ...
  const cargar = async () => {
    setLoading(true);
    try {
      const res  = await authFetch(`/billetera/movimientos?anio=${anio}`, user.token);
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400">Movimientos registrados en tu billetera</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-gray-300">
          ...
          <p className="text-sm font-medium text-gray-400">Sin movimientos en {anio}</p>
        </div>
      ) : (
        <div className="bg-white border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-4 px-6 py-3 bg-gray-50 border-b border-gray-100">
            ...
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Entrada</p>
              ...
              <div key={mes}>
                {movs.map((mov, idx) => {
                  ...
                  const fechaLabel = partes.length === 3
                    ...
                    : "—";
                  const cat = mov.entrada?.categoria;
                  ...
                  return (
                    <div key={mov.id}
                      ...
                      <p className="text-sm text-gray-700 truncate pr-4">
                        {mov.entrada?.nombre ?? mov.descripcion ?? "—"}
                      </p>
                      <div>
                        {cat ? (
                ...
                <div className="grid grid-cols-4 px-6 py-2 bg-purple-50 border-b border-purple-100">
                  <p className="text-xs font-bold text-purple-500 col-span-3">
                    Subtotal {MESES[parseInt(mes)]}
                  </p>
                  <p className="text-xs font-bold text-purple-700 text-right">
                    {fmt(totalMes, user.currency)}
                  </p>
                </div>
              </div>
            );
          ...
          <div className="grid grid-cols-4 px-6 py-4 bg-purple-600">
            <p className="text-sm font-bold text-purple-200 col-span-3 flex items-center gap-2">
              <Wallet size={14} /> Total {anio}
            </p>
            <p className="text-base font-bold text-white text-right">
              {fmt(totalAnio, user.currency)}
            </p>
          </div>
