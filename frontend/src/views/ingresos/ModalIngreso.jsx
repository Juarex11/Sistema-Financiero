...
export default function ModalIngreso({ user, onClose, onSaved, config, obligatorio = false, mesNombre = "" }) {
  ...
  return createPortal(
    <div
      ...
      onMouseDown={(e) => !obligatorio && e.target === e.currentTarget && onClose()}
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h3 className="font-bold text-gray-800">
            {obligatorio ? `¿Cuánto ganaste en ${mesNombre}?` : "Registrar ingreso"}
          </h3>
          {!obligatorio && (
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            {config?.monto_base && (
              <p className="text-xs text-purple-500 font-semibold mb-2">
                Sugerido: {new Intl.NumberFormat("es-PE", { style: "currency", currency: user.currency }).format(config.monto_base)} — puedes modificarlo
              </p>
            )}
          ...
          <div>
            ...
            <input
              type="text" placeholder="Ej: Sueldo de mayo"
        ...
        <div className="flex gap-3 px-6 pb-6">
          {!obligatorio && (
            <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-500 text-sm font-semibold hover:bg-gray-50">
              Cancelar
            </button>
          )}
          <button onClick={handleSave} disabled={saving}
            ...
            {saving ? "Guardando…" : obligatorio ? "Confirmar ingreso" : "Guardar"}
          </button>
        </div>
      </div>
    </div>,
